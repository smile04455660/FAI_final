...
class MyPlayer(BasePokerPlayer):
    ...
    def _encode_actor_inputs(self, valid_actions, hole_card, round_state):
        ...
        all_cards = torch.bitwise_or(private_cards, community_cards)
        ...
        my_i, my_seat = [(i, seat) for (i, seat) in
                         enumerate(round_state['seats']) if seat['uuid'] == self.uuid][0]
        enemy_stack = self._encode_number(1, enemy_seat['stack'])
        my_stack = self._encode_number(1, my_seat['stack'])
        ...
        ret = torch.cat([private_cards, community_cards, all_cards, rounds_left, bigblind_amt,
    ...
    def _encode_history(self, bigblind_amt, number, history):
        ...
        for i in range(number):
            ret.append(self._encode_bet(bigblind_amt, history['amount']))
